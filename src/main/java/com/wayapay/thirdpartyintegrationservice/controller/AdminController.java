...
public class AdminController {
    ...
    public ResponseEntity<?> searchAndFilterTransactionStatus(@PathVariable("transaction_status") boolean transaction_status, @RequestParam(required = false, defaultValue = "0") String pageNumber,
            @RequestParam(required = false, defaultValue = "10") String pageSize, @RequestHeader("Authorization") String token) {

        ResponseEntity<?> transactionDetailPage = billsPaymentService.searchAndFilterTransactionStatus(transaction_status, Integer.parseInt(pageNumber), Integer.parseInt(pageSize), token);
    ...
    @ApiOperation(value = "Get all Transaction Report : This API is used to get all transaction report failed or successful")
    @ApiResponses(value = {
        @ApiResponse(code = 200, message = "Successful")
    })
    @GetMapping("/biller/report/all-transactions")
    public ResponseEntity<?> allTransactionReport(@PathVariable("transaction_status") boolean transaction_status, @RequestParam(required = false, defaultValue = "0") String pageNumber,
            @RequestParam(required = false, defaultValue = "10") String pageSize,
            @RequestHeader("Authorization") String token,
            @RequestParam("fromdate") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate fromdate,
            @RequestParam("todate") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate todate) {

        ResponseEntity<?> transactionDetailPage = billsPaymentService.getAllTransactions(Integer.parseInt(pageNumber), Integer.parseInt(pageSize), fromdate, todate);
        return ResponseEntity.ok(new SuccessResponse(transactionDetailPage));
    }
